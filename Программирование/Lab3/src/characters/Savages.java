...
public class Savages extends IslandVisitor implements Attackable {
    ...
    public Savages(String name) throws NameException {
        super(name);
    }
    ...
    public void act(LocalDate date, Character c) {
        ...
        if (this.getActivity() != VisitorActivity.ISLAND_ABSENCE && date.equals(departureDate)) {
            leave(date);
        }

        if (Math.random() < 0.5 && date.isAfter(departureDate)) {
            visit(date);
