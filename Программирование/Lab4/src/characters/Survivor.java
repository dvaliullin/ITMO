...
public class Survivor extends IslandVisitor implements Dreamable, Attackable  {
    ...
    public Survivor(String name, LocalDate arrivalDate, int stayDuration) throws NameException {
    ...
    public void act(LocalDate date, Character c) throws ScenarioException {
        ...
        if (date.isAfter(getArrivalDate().plusYears(1).plusMonths(3)) && savages.isOnIsland
        && Math.random() < 0.5) {
            seeVisitor(savages);
        }

        if (date.equals(getArrivalDate().plusYears(23)) && date.getMonth().equals(Month.MAY)) {
        ...
        if (date.equals(getDepartureDate())) {
            leave(date);
        }
    }
